...
- IPv4 and IPv6 use independent APNIC/BGP-derived projects. Duplicate, overlapping, and adjacent ranges are collapsed without changing their address union.
- Domain coverage combines V2Fly's structured `cn` data with felixonmars' actively maintained mainland DNS-routing list. Commented-out dnsmasq entries are treated as removed and never imported.
- ASN data is parsed only from explicit `IP-ASN,<number>` records in the daily ASN-China snapshot.
- A disappeared domain or ASN remains for three Beijing-calendar-day observations on which every configured source refreshed successfully. Re-running the updater on the same day or any source failure does not age it toward deletion.
...
Instead, a rule becomes eligible for removal only after all configured successful source snapshots stop reporting it for three distinct observations. Source download failures do not count. The update report records additions, retained missing rules, and retirements.
