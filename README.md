...
5. 仅在内容变化时把 `upstream/` 与 `dist/` 提交并推送到 `main`；
6. 通过 `main` 分支下 `dist/` 的固定 Raw 地址持续发布规则。

自动生成文件不可直接编辑。人工域名变更应修改 `rules/`；自动快照位于 `upstream/`；数据源和数量保护阈值位于 `config/`。严格覆盖档位会过滤容易误伤全球网站的开放顶级域；最大覆盖档位优先覆盖率，误分流风险略高。

## 发布策略

每个通过验证的数据快照直接提交到 `main`，所有客户端通过 `dist/` 下不变的 Raw 地址获取最新规则。自动更新不会创建 GitHub Release，也不会创建 `rules-YYYYMMDD-xxxx` 形式或其他形式的版本标签。严格/最大覆盖是同一数据快照的覆盖档位，各客户端文件是兼容产物，不把它们当成彼此竞争的“版本”。Shadowrocket 独立完整模块同样由这一份规则模型生成，不维护第二套手工规则。
